Базовый глагол **go** — один из самых частотных. С разными частицами охватывает движение, изменения, события и решения.

← [[Phrasal Verbs Dictionary — Introduction|Назад к словарю]]

---

| # | Фразовый глагол | Перевод | Тип | Пример | Заметки |
|---|---|---|---|---|---|
| 1 | **go on** | продолжать(ся) | intrans | Go on, I'm listening. / The meeting went on until midnight. | |
| 2 | **go on** | происходить | intrans | What's going on here? | |
| 3 | **go on** | продолжать делать (+ герундий) | intrans | He went on talking for an hour. | |
| 4 | **go off** | сработать (будильник, сигнализация) | intrans | The fire alarm went off. | |
| 5 | **go off** | испортиться (еда) | intrans | This cheese has gone off. | |
| 6 | **go off** | взорваться | intrans | The bomb went off. | |
| 7 | **go off** | разонравиться | insep | I've gone off chocolate. | BrE |
| 8 | **go out** | выйти (из дома, на вечер) | intrans | Let's go out tonight. / We go out every Friday. | ср. go away (уехать надолго) |
| 9 | **go out** | погаснуть | intrans | The lights went out. | |
| 10 | **go out** | встречаться | intrans | They've been going out for a year. | |
| 11 | **go back** | возвращаться | intrans | We went back to the hotel. | |
| 12 | **go through** | пережить | insep | She went through a difficult time. | |
| 13 | **go through** | просмотреть | insep | Let's go through the report. | |
| 14 | **go ahead** | начинать; действовать | intrans | Go ahead, I'm listening. / The project will go ahead as planned. | |
| 15 | **go over** | повторить; просмотреть | insep | Let's go over the main points again. | |
| 16 | **go up** | подниматься; расти (цена) | intrans | Prices have gone up again. | |
| 17 | **go down** | снижаться; садиться (солнце) | intrans | The temperature went down. / The sun went down at 8. | |
| 18 | **go away** | уехать; уйти; исчезнуть | intrans | Go away! / The pain won't go away. | ср. go out (выйти ненадолго) |
