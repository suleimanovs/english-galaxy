Базовый глагол **take** охватывает широкий спектр значений: от буквального «снять» до «взять на себя», «увлечься» и «пойти в кого-то».

← [[Phrasal Verbs Dictionary — Introduction|Назад к словарю]]

---

| # | Фразовый глагол | Перевод | Тип | Пример | Заметки |
|---|---|---|---|---|---|
| 1 | **take off** | снять (одежду) | sep | Take off your shoes. / Take it off. | |
| 2 | **take off** | взлететь | intrans | The plane took off on time. | |
| 3 | **take off** | стать популярным | intrans | Her career took off. / The idea really took off. | |
| 4 | **take on** | взять на себя (работу) | sep | She took on too much responsibility. | |
| 5 | **take on** | нанять | sep | We're taking on new staff. | |
| 6 | **take up** | начать заниматься (хобби) | sep | He took up painting. | |
| 7 | **take up** | занимать (место/время) | sep | This table takes up too much space. | |
| 8 | **take over** | взять под контроль | sep | A large company took over the business. | |
| 9 | **take over** | принять дела | intrans | Can you take over while I'm away? | |
| 10 | **take out** | вынуть | sep | Take out your books. | |
| 11 | **take out** | пригласить куда-то | sep | He took her out to dinner. | |
| 12 | **take in** | приютить | sep | She took in a stray cat. / She took in a homeless man. | |
| 13 | **take in** | обмануть | sep | Don't be taken in by his lies. | часто в passive |
| 14 | **take in** | усвоить (информацию) | sep | It's a lot to take in. / There was so much to take in. | |
| 15 | **take in** | ушить (одежду) | sep | I need to take in this dress. | |
| 16 | **take back** | вернуть (в магазин) | sep | I took the shirt back to the shop. | |
| 17 | **take back** | взять слова обратно | sep | I take back what I said. | |
| 18 | **take after** | быть похожим на (родственника) | insep | She takes after her mother. | характер и/или внешность; ср. look like (только внешность, с кем угодно) |
| 19 | **take down** | записать | sep | Take down this number. | |
| 20 | **take down** | снять (со стены) | sep | They took down the poster. | |
| 21 | **take apart** | разобрать (на части) | sep | He took apart the engine to find the problem. | |
| 22 | **take to** | полюбить; привыкнуть к | insep | She took to him immediately. / He's taken to jogging every morning. | |
